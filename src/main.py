...
def get_station():
    ...
    station = undergroundStation(config.station_url)
...
def get_last_time_station_requested():
    ...
    return config.station_url['updated_on']
...
def build_arrival_time(display, arrival, row_num):
    ...
    display.text((DISPLAY_WIDTH - (w_time + w_text), ((row_num - 1) * 14)), text=arrival_time + arrival_text,
                 font=font_regular, fill="yellow")
...
try:
    ...
    while True:
        ...
        if hasattr(requested_station, 'id'):
            ...
            # Refresh Data when TFL Expired or Forced Refresh
